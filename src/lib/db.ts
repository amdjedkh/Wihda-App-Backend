...
export async function getNeighborhoodById(
  ...
  const result = await db
    .prepare("SELECT * FROM neighborhoods WHERE id = ?")
    .bind(id)
    .first<Neighborhood>();
  return result;
}
...
export async function getNeighborhoodsByCity(
  ...
  const result = await db
    .prepare("SELECT * FROM neighborhoods WHERE city = ? AND is_active = 1")
    .bind(city)
    .all<Neighborhood>();
...
export async function getNeighborhoodsNearLocation(
  ...
  const result = await db
    .prepare("SELECT * FROM neighborhoods WHERE is_active = 1")
    .all<Neighborhood>();
    ...
    const distance =
      Math.sqrt(
        Math.pow((n.center_lat - lat) * 111, 2) +
          Math.pow(
            (n.center_lng - lng) * 111 * Math.cos((lat * Math.PI) / 180),
            2,
          ),
      ) * 1000;
...
export async function joinNeighborhood(
  ...
  `,
    ...
    .bind(id, userId, neighborhoodId, now)
    .run();
...
export async function getCoinRule(
  db: D1Database,
  sourceType: string,
): Promise<CoinRule | null> {
  const result = await db
    .prepare("SELECT * FROM coin_rules WHERE source_type = ? AND is_active = 1")
    .bind(sourceType)
    .first<CoinRule>();
  return result;
}
...
export async function getLeftoverOfferById(
  db: D1Database,
  id: string,
): Promise<LeftoverOffer | null> {
  return await db
    .prepare("SELECT * FROM leftover_offers WHERE id = ?")
    .bind(id)
    .first<LeftoverOffer>();
}
...
export async function updateLeftoverOfferStatus(
  ...
  await db
    .prepare(
      "UPDATE leftover_offers SET status = ?, updated_at = ? WHERE id = ?",
    )
    .bind(status, now, id)
    .run();
}
...
export async function getLeftoverNeedById(
  db: D1Database,
  id: string,
): Promise<LeftoverNeed | null> {
  return await db
    .prepare("SELECT * FROM leftover_needs WHERE id = ?")
    .bind(id)
    .first<LeftoverNeed>();
}
...
export async function updateLeftoverNeedStatus(
  ...
  await db
    .prepare(
      "UPDATE leftover_needs SET status = ?, updated_at = ? WHERE id = ?",
    )
    .bind(status, now, id)
    .run();
}
...
export async function getMatchesForUser(
  ...
  let query =
    "SELECT * FROM matches WHERE (offer_user_id = ? OR need_user_id = ?)";
...
export async function getChatThreadByMatchId(
  ...
  return await db
    .prepare("SELECT * FROM chat_threads WHERE match_id = ?")
    .bind(matchId)
    .first<ChatThread>();
}
...
export async function createChatMessage(
  ...
  return (await db
    ...
    .bind(id)
    .first<ChatMessage>()) as ChatMessage;
}
...
export async function getCleanifySubmissionById(
  db: D1Database,
  id: string,
): Promise<CleanifySubmission | null> {
  return await db
    .prepare("SELECT * FROM cleanify_submissions WHERE id = ?")
    .bind(id)
    .first<CleanifySubmission>();
}
  ...
  try {
    ...
    if ((error as Error).message?.includes("UNIQUE constraint failed")) {
      ...
      const existing = await db
        .prepare("SELECT * FROM campaigns WHERE source = ? AND url = ?")
        .bind(data.source, data.url)
        .first<Campaign>();
...
export async function getCampaignsForNeighborhood(
  ...
  let query =
    "SELECT * FROM campaigns WHERE neighborhood_id = ? AND status = ?";
...
/**
 * Update a user's verification_status column.
 * Called by the webhook route after AI/admin review.
 */
...
/** Create a new verification session for a user with a 24-hour TTL. */
...
/**
 * Fetch the most recent verification session for a user.
 * Returns null if the user has no sessions at all.
 */
export async function getLatestVerificationSessionForUser(
  db: D1Database,
  userId: string,
): Promise<VerificationSession | null> {
  return db
    .prepare(
      "SELECT * FROM verification_sessions WHERE user_id = ? ORDER BY created_at DESC LIMIT 1",
    )
    .bind(userId)
    .first<VerificationSession>();
}

/** Partial update helper — only non-undefined fields are written. */
export async function updateVerificationSession(
  ...
  await db
    .prepare(
      `UPDATE verification_sessions SET ${setClauses}, updated_at = datetime('now') WHERE id = ?`,
    )
